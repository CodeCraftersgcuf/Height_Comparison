...
const page = () => {
    ...
    const validationSchema = Yup.object({
        name: Yup.string().min(3,'Must be at least 3 characters').max(15,'Must be max 15 characters').required("name is required"),
        email: Yup.string().email("Invalid email").required("Email is required"),
        password: Yup.string().min(6, "Must be at least 6 characters").max(15,"Must be max 15 characters").required("Password is required"),
    });
    ...
    return (
        <div className="flex justify-center items-center min-h-screen bg-gray-100 py-8">
            <div className="bg-white shadow-lg rounded-lg p-6 w-96 ">
                ...
                {/* Sign-in Buttons */}
                <h1 className="text-xl font-bold tracking-widest mb-4 block">Sign up</h1>
                ...
                <button className="border border-gray-300 w-full flex items-center justify-center py-2 rounded-md hover:bg-gray-100">
                    <i className="bi bi-apple mr-2"></i> Sign up with Apple
                </button>
